...
const format = ([server, count]) => `\`${count}\` [${server}](https://widgetbot.io/channels/${server})`
const join = (servers: [string, number][]) => servers.reduce((prev, server) => `${prev}${format(server)}\n`, '')
