...
class Datastore extends EventEmitter {
  ...
  find(query = {}, projection) {
    ...
    return new Cursor(this.__original.find(query, projection), this.load(), (error, result) => {
      if (error) {
      ...
      return new Promise((resolve, reject) => {
        this.__original.update(query, update, options, (error, numAffected, affectedDocuments, upsert) => {
          if (error) {
            ...
            let result = options.returnUpdatedDocs ? affectedDocuments : numAffected
  ...
  static create(options) {
    return new Proxy(new this(options), {
      ...
      set(target, key, value) {
        return target.__original.hasOwnProperty(key) ? (target.__original[key] = value) : (target[key] = value)
      }
    })
  }
}
